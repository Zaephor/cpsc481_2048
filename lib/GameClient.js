...
GameClient.move = function (gamestate, direction) {
  ...
  return Promise.resolve().then(function () {
    return StatTurn.update({algorithm: gamestate.algorithm, turn: gamestate.turn}, {
      $max: {maxBoardScore: logEntry.score, maxBoardValue: boardValue, maxTile: _.max(logEntry.uniqTiles)},
      $min: {minBoardScore: logEntry.score, minBoardValue: boardValue, minTile: _.min(logEntry.uniqTiles)},
      $inc: {
        hits: 1,
      ...
      return StatTurn
        .update({turn: gamestate.turn, algorithm: 'all'}, {
          $max: {maxBoardScore: logEntry.score, maxBoardValue: boardValue, maxTile: _.max(logEntry.uniqTiles)},
          $min: {minBoardScore: logEntry.score, minBoardValue: boardValue, minTile: _.min(logEntry.uniqTiles)},
          $inc: {
            hits: 1,
